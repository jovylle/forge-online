...
export default function DashboardError({
  ...
  const genericMessage =
    "The dashboard request failed on the server. Retry, and check server logs if the issue persists.";
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,_#09090b,_#111827_40%,_#020617)] px-6 py-10 text-zinc-100">
      <div className="mx-auto max-w-3xl rounded-[2rem] border border-rose-500/20 bg-rose-500/10 p-8">
        ...
        {error.digest ? (
          <p className="mt-2 text-xs text-rose-200/80">Error digest: {error.digest}</p>
        ) : null}
        <button
          type="button"
          onClick={reset}
